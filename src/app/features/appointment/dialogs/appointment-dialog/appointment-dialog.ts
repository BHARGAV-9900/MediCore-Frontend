...
export class AppointmentDialog implements OnInit {
  ...
  /*
   * Converts selected date + 12-hour time
   * into a Date object.
   */
  private buildAppointmentDate(): Date {
    ...
    const hour =
      this.form.value.appointmentHour!;

    const minute =
      Number(
        this.form.value.appointmentMinute!
      );

    const period =
      this.form.value.appointmentPeriod!;
 ...
 createAppointment(): void {
  ...
  const appointmentDate =
    this.buildAppointmentDateString();
